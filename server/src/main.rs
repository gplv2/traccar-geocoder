...
struct Index {
    ...
    max_distance_sq: f64,
    country_boundaries: Option<CountryBoundaries>,
...
impl Index {
    fn load(
        ...
        Ok(Index {
            ...
            admin_cell_level,
            max_distance_sq,
            country_boundaries: if enable_country_fallback {
                Some(
    ...
    fn query(&self, lat: f64, lng: f64) -> Address<'_> {
        ...
        if admin.country.is_none() {
            if let Some(ref cb) = self.country_boundaries {
                if let Ok(latlon) = LatLon::new(lat, lng) {
                    for id in cb.ids(latlon) {
                        if id.len() == 2 {
...
async fn main() {
    ...
    let no_country_fallback = args.iter().any(|a| a == "--no-country-fallback");

    let db_path = format!("{}/geocoder.json", data_dir);
    let db = auth::Db::load(&db_path);

    eprintln!("Loading index from {}...", data_dir);
    let enable_country_fallback = !no_country_fallback;
    if enable_country_fallback {
        eprintln!("Country boundary fallback enabled (disable with --no-country-fallback)");
    }
